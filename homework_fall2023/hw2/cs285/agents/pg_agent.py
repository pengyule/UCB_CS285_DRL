...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        info: dict = self.actor.update(obs, actions, terminals, advantages)
    ...
    def _estimate_advantage(
        ...
        if self.normalize_advantages:
            ...
            if advantages_std > 0:
                ...
                # If std is zero, we can just return the advantages as they are
                pass
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        # Does rewards (Sequence[float]) support an np array? or should it be a list?
        # in fact, 
        gamma_powers = self.gamma ** np.arange(len(rewards))
        discounted_return = (rewards * gamma_powers).sum()
        # return an np array with the same value repeated for each entry
        return np.full(len(rewards), discounted_return)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        sum = 0
        # ret should be an np array with the same length as rewards
        ret = []
