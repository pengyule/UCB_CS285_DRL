...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(env, actor, params['batch_size'], params['ep_len'], render=log_video)
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          perm = np.random.permutation(len(replay_buffer.obs))
          perm = perm[:params['train_batch_size']]
          ob_batch, ac_batch = ptu.from_numpy(replay_buffer.obs[perm]), ptu.from_numpy(replay_buffer.acs[perm])
